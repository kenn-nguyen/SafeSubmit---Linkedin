...
export const FileUpload: React.FC<FileUploadProps> = ({ 
  ...
  const validateAndSetFile = (file: File) => {
    ...
    // Check if valid via extension OR mime type
    ...
    if (!isValid) {
         setError(`Invalid file type. Accepted: ${accept.replace(/,/g, ', ')}`);
         return;
    }
  ...
  // Safe ID generation for the input
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center justify-center gap-3">
        ...
        {error && (
          <div className="flex items-center gap-2 text-red-600 text-xs mt-2 font-medium animate-in fade-in slide-in-from-top-1">
            <AlertCircle size={12} />
            <span>{error}</span>
          </div>
        )}
