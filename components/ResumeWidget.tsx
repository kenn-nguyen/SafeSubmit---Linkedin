...
import { FileText, RefreshCw, Lock, Trash2, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        disabled={isDisabled}
        className={`
            flex items-center gap-2 px-3 py-2.5 border rounded-lg transition-all shadow-sm
            ${isConfirming 
                ? 'bg-red-50 border-red-200 text-red-600 w-[120px] justify-center' 
                : 'bg-white border-gray-200 text-gray-400 hover:text-red-600 hover:bg-red-50 hover:border-red-200'
            }
            disabled:opacity-50 disabled:cursor-not-allowed
        `}
        title={isConfirming ? "Click again to confirm" : "Reset Session (Delete Resume & Data)"}
      >
        {isConfirming ? (
            <><AlertCircle size={16} /><span className="text-xs font-bold">Confirm?</span></>
        ) : (
            <Trash2 size={16} />
        )}
      </button>
